...
#pragma region ExtensionsInterface
	...
	virtual void BeforePoseSolve(const FMediaPipeFrame& Frame, FComponentSpacePoseContext& Context, int CurrentLOD) override;
...
private:
	...
	void InitJoints();
	void Initialize(const FComponentSpacePoseContext& Context, int AnimationLOD);
	...
	bool GetJointTransform(const FName& BoneName, FComponentSpacePoseContext& Context, FTransform& BoneTransform) const;
	bool WriteJointMotion(const FName& JointName, std::function<bool(const FBvhBoneCacheItem& BoneItem, FTransform& OutTransform)> TransformGetter);
	TSharedPtr<IBvHWriter> Writer;
	FTransform ComponentTransform;
	TMap<FName, FBvhBoneCacheItem> Bones;
	TWeakObjectPtr<USkeletalMeshComponent> SkeletalMeshComponent;
	TMap<EHumanoidBones, FName> BoneNameMapping;
	FCriticalSection InitializationCritical;
