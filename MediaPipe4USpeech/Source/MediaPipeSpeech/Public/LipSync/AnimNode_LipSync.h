...
#include "OculusLipSyncRemapAsset.h"
...
struct MEDIAPIPESPEECH_API FAnimNode_LipSync : public FAnimNode_Base
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "LipSync", meta=(PinShownByDefault))
	TObjectPtr<UOculusLipSyncRemapAsset> RetargetAsset;
...
private:
	bool GetCurrenCurveValue(FPoseContext& PoseContext, const FName& CurveName, float& ValueOut);
	void ProcessCurveWMAOperation(FPoseContext& Output, const FName& Name, float CurrentValue, float& InOutLastValue);
	void ProcessCurveOperation(FPoseContext& Output, const FName& Name, float CurrentValue, float NewValue);
