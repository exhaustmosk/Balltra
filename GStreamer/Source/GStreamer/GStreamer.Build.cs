...
public class GStreamer : ModuleRules
{
    ...
    public GStreamer(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        if (Target.Platform == UnrealTargetPlatform.Win64)
        {
            ...
            if (string.IsNullOrWhiteSpace(GStreamerRoot))
            {
                Log.WriteLine(LogEventType.Warning, "Miss GSTREAMER_1_0_ROOT_MSVC_X86_64 or GSTREAMER_ROOT environment variables.");
                return;
            }
            ...
            Log.WriteLine(LogEventType.Log, $"GStreamer path: {Environment.NewLine}{GStreamerRoot}");
    ...
    private void CopyDirectory(string sourceDir, string destinationDir, bool recursive)
    {
        if (Directory.Exists(sourceDir))
        {
            foreach (string filePath in Directory.GetFiles(sourceDir, "*.*", SearchOption.AllDirectories))
            {
                ...
                if (!IgnoreExtensions.Contains(ext, StringComparer.OrdinalIgnoreCase))
                {
                    string relativePah = Path.GetRelativePath(sourceDir, filePath);
                    Log.WriteLine(LogEventType.Log, $"Find File: {Path.Combine(destinationDir, relativePah)}");
